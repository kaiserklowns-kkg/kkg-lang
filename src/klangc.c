...
static void codegen(SB *sb) {
    sb_init(&g_js_lib);
    sb_append(&g_js_lib, "// Generated by klangc 0.19 — the JavaScript half of this\n"
                         "// program's 'js fn' declarations. Pass it to emcc with\n"
                         "// --js-library; do not edit by hand.\n"
                         "mergeInto(LibraryManager.library, {\n");
    sb_append(sb, "/* Generated by klangc 0.19 — do not edit by hand */\n");
...
static void write_default_index(const char *path, const char *title) {
    ...
    fprintf(f,
        ...
        "</style>\n"
        "\n"
        "<!-- Generated by `klangc web run`, because this project has no index.html.\n"
        "     Write your own next to the .kkg file and that one will be used. -->\n"
        "<h1>%s</h1>\n"
        "<div id=\"app\"></div>\n"
        "\n"
...
static int cmd_web(int argc, char **argv) {
    ...
    /* Where the page lives: beside the program, in a directory named after it, or
       in the project's web/ — which is the layout `klangc new` lays down. Failing
       all three, the build directory gets a plain generated one. */
    ...
    if (!found) {
        snprintf(probe, sizeof probe, "%s/%s/index.html", dir, stem);
        if (file_exists(probe)) { snprintf(root, sizeof root, "%s/%s", dir, stem); found = root; }
    }
    if (!found && file_exists("web/index.html")) { snprintf(root, sizeof root, "web"); found = root; }
    if (!found) {
        snprintf(root, sizeof root, ".klang/%s", stem);
        ensure_dir(root);
        snprintf(probe, sizeof probe, "%s/index.html", root);
        if (!file_exists(probe)) {
            write_default_index(probe, stem);
            printf("klangc: wrote '%s' - a starting page, yours to replace\n", probe);
        }
    }
...
static const char *TEMPLATE_WEB =
    "// A Klang web page. Everything here - state, rendering, events - is Klang;\n"
...
/* The stylesheet is a file rather than a <style> block, so that replacing it with
   a real one — Tailwind's output, or anything else — changes nothing else. */
static const char *TEMPLATE_WEB_HTML =
    "<!doctype html>\n"
    "<meta charset=\"utf-8\">\n"
    ...
    "\n"
    "<h1>%s</h1>\n"
    "<p>The page is yours. The logic is in <code>src/main.kkg</code>.</p>\n"
    "\n"
    "<button id=\"button\">click me</button>\n"
    "<span id=\"count\"></span>\n"
    "\n"
    "<!-- klangc builds app.js and app.wasm next to this file. -->\n"
    "<script src=\"app.js\"></script>\n";

static const char *TEMPLATE_WEB_CSS =
    "/* Ordinary CSS. The dev server sends it as-is; nothing compiles it. */\n"
    ...
    "\n"
    "#count { display: block; margin-top: 1rem; opacity: .7; }\n";
...
static const char *TEMPLATE_WEB_TW =
    ...
    "//     klangc web run\n"
    "//\n"
    "// The utility classes below live in Klang string literals. `web/input.css`\n"
    ...
    "    dom.setHtml(\"#count\",\n"
    "        \"<span class=\\\"rounded-full px-3 py-1 text-sm ${tone}\\\">${clicks} clicks</span>\")\n"
    "}\n"
    "\n"
    "// JavaScript can call an `export fn` by name - which is how a button reaches\n"
    "// back into Klang. Its arguments and result have to be types JavaScript\n"
...
static const char *TEMPLATE_TAILWIND_HTML =
    ...
    "    The page is yours. The logic is in <code>src/main.kkg</code>.\n"
    "  </p>\n"
    "\n"
    "  <button id=\"button\"\n"
    "          class=\"mt-6 rounded border border-gray-400/50 px-4 py-2 hover:bg-gray-500/10\">\n"
    "    click me\n"
    "  </button>\n"
    "  <span id=\"count\" class=\"ml-3 text-sm opacity-70\"></span>\n"
    "\n"
    "  <script src=\"app.js\"></script>\n"
    "</body>\n";
...
static int cmd_new(int argc, char **argv) {
    ...
    if (web) {
        ...
        ensure_dir(path);
        const char *tpl = tailwind ? TEMPLATE_TAILWIND_HTML : TEMPLATE_WEB_HTML;
        snprintf(path, sizeof path, "%s/web/index.html", name);
        char *html = malloc(strlen(tpl) + 2 * strlen(name) + 8);
        sprintf(html, tpl, name, name);
        ...
        if (tailwind) {
            ...
            snprintf(path, sizeof path, "%s/web/style.css", name);
            write_file(path, TEMPLATE_WEB_CSS);
        }
    }
...
static int cmd_run(int argc, char **argv) {
    const char *entry = find_entry(argc > 0 && argv[0][0] != '-' ? argv[0] : NULL);
    char *stem = stem_of(entry);

    ensure_dir(".klang");
    char cpath[512], exe[512];
    snprintf(cpath, sizeof cpath, ".klang/%s.c", stem);
#ifdef _WIN32
    snprintf(exe, sizeof exe, ".klang/%s.exe", stem);
#else
    snprintf(exe, sizeof exe, ".klang/%s", stem);
#endif
    ...
    if (lib) {
        fprintf(stderr,
            "klangc: '%s' has `js fn` declarations, so it needs a browser\n"
            "       | help: run it with `klangc web run %s`\n", entry, entry);
        return 1;
    }
    ...
    if (!cc) {
        fprintf(stderr,
            "klangc: no C compiler on PATH, and Klang compiles through C\n"
            "       | help: install gcc or clang\n");
        return 1;
    }

    char cmd[2048];
    int n = snprintf(cmd, sizeof cmd, "%s -std=c99 -O2 -o %s %s", cc, exe, cpath);
    for (int i = 0; i < g_c_links.count; i++)
    ...
    char runline[600];
#ifdef _WIN32
    snprintf(runline, sizeof runline, "%s", exe);
#else
    snprintf(runline, sizeof runline, "./%s", exe);
#endif
...
int main(int argc, char **argv) {
    ...
    if (argc < 2 || strcmp(argv[1], "--help") == 0 || strcmp(argv[1], "-h") == 0) {
        printf("klangc - the Klang compiler (0.19)\n\n"
               "Start something:\n"
               ...
               "\n"
               "Run it:\n"
               "  klangc run [file.kkg]            Compile, build and execute\n"
    ...
    if (strcmp(argv[1], "--version") == 0) { printf("klangc 0.19.0\n"); return 0; }
