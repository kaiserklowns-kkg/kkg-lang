...
static int cmd_new(int argc, char **argv) {
    ...
    snprintf(path, sizeof path, "%s/src/main.kkg", name);
    write_file(path, web ? TEMPLATE_WEB : cli ? TEMPLATE_CLI : TEMPLATE_SERVER);

    if (web) {
        snprintf(path, sizeof path, "%s/web", name);
        ensure_dir(path);
        snprintf(path, sizeof path, "%s/web/index.html", name);
        char *html = malloc(strlen(TEMPLATE_WEB_HTML) + 2 * strlen(name) + 8);
        sprintf(html, TEMPLATE_WEB_HTML, name, name);
        write_file(path, html);
        free(html);
    }

    snprintf(path, sizeof path, "%s/.gitignore", name);
    write_file(path, GITIGNORE_TEMPLATE);
