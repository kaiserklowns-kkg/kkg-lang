...
static void codegen(SB *sb) {
    sb_init(&g_js_lib);
    sb_append(&g_js_lib, "// Generated by klangc 0.24 — the JavaScript half of this\n"
                         "// program's 'js fn' declarations. Pass it to emcc with\n"
                         "// --js-library; do not edit by hand.\n"
                         "mergeInto(LibraryManager.library, {\n");
    sb_append(sb, "/* Generated by klangc 0.24 — do not edit by hand */\n");
...
static void write_default_index(const char *path, const char *title, const char *root) {
    ...
    fprintf(f,
        ...
        "\n"
        "     Nothing needs to go in here. The markup comes from std/ui and the\n"
...
static const char *TEMPLATE_WEB =
    ...
    "import \"std/ui\"\n"
    "\n"
    "// State is ordinary Klang. It lives at module level because a handler runs\n"
    "// long after main returned and cannot capture a local.\n"
    "let mut clicks = 0\n"
    "\n"
    "// A component is a function that returns a Node. Handlers are closures, so\n"
    "// the button carries its own behaviour - no id, no wiring elsewhere.\n"
    "fn view() -> ui.Node {\n"
    "    let tone = if clicks == 0 { \"bg-gray-500/10\" } else { \"bg-emerald-500/20\" }\n"
    "    return ui.col([\n"
    "        ui.with(ui.h1(\"Hello from Klang\"), [ui.class(\"text-2xl font-semibold\")]),\n"
    "        ui.with(ui.line(\"The page is yours. It is built in src/main.kkg.\"),\n"
    "                [ui.class(\"mt-1 text-sm opacity-70\")]),\n"
    "        ui.with(ui.button(\"click me\", || { clicks += 1; ui.refresh() }),\n"
    "                [ui.class(\"mt-6 rounded border border-gray-400/50 px-4 py-2 \"\n"
    "                        + \"hover:bg-gray-500/10\")]),\n"
    "        ui.with(ui.tag(\"span\", \"${clicks} clicks\"),\n"
    "                [ui.id(\"count\"),\n"
    "                 ui.class(\"ml-3 rounded-full px-3 py-1 text-sm ${tone}\")]),\n"
    "    ])\n"
    "}\n"
    "\n"
    "fn main() {\n"
...
static int cmd_new(int argc, char **argv) {
    ...
    /* Styling is Tailwind. There was a choice here and it was a mistake: three
       ways to style a page meant no way was the way, and every example picked a
       different one. */
    bool tailwind = web;
    ...
    if (web) {
        ...
        sb_append(&rm,
            ...
            "Tailwind at `src/**/*.kkg` as well as the page, because the class names\n"
            "live in Klang string literals and Tailwind would not look there.\n\n");
        sb_append(&rm,
...
int main(int argc, char **argv) {
    ...
    if (argc < 2 || strcmp(argv[1], "--help") == 0 || strcmp(argv[1], "-h") == 0) {
        printf("klangc - the Klang compiler (0.24)\n\n"
               "Start something:\n"
    ...
    if (strcmp(argv[1], "--version") == 0) { printf("klangc 0.24.0\n"); return 0; }
